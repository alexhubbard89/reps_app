def get_vote_menu(congress_num, session_num):
	...
	columns_needed = ['issue', 'question', 'questionmeasure', 'result', 'title',
	...
	url = 'http://www.senate.gov/legislative/LIS/roll_call_lists/vote_menu_{}_{}.xml'.format(congress_num, 
                                                                                            session_num)
	r = requests.get(url)
	year = json_normalize(pd.DataFrame(bf.data(fromstring(r.content))
                                  ).loc['congress_year','vote_summary']).loc[0,'$']
	...
	x.loc[:,'vote_date'] = x.loc[:,'vote_date'].apply(
		lambda x: datetime.datetime.strptime('{}-{}'.format(x,year), '%d-%b-%Y'))
	x['vote_date'] = pd.to_datetime(x['vote_date']).apply(pd.datetools.normalize_date)
	...
	try:
		## This is needed because the format change
		## for a little.
	    x.loc[x['issue'].isnull(), 'issue'] = None
	    x.loc[x['issueA'].isnull(), 'issueA'] = None
	    x['questionmeasure'] = x['questionA']

	    for i in range(len(x)):
	        if x.loc[i, 'issue'] == None:
	            x.loc[i, 'issue'] = x.loc[i, 'issueA']
	except:
	    'dont need to clean'
